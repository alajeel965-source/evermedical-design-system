...
-- 4. Create separate policies for INSERT, UPDATE, DELETE operations
DROP POLICY IF EXISTS "Prevent unauthorized event insertions" ON public.medical_events;
DROP POLICY IF EXISTS "Prevent unauthorized event updates" ON public.medical_events;
DROP POLICY IF EXISTS "Prevent unauthorized event deletions" ON public.medical_events;

CREATE POLICY "Prevent unauthorized event insertions" 
ON public.medical_events 
FOR INSERT 
TO authenticated
WITH CHECK (
  created_by = auth.uid()  -- Only creators can insert their events
  OR public.is_current_user_verified_admin()  -- Or verified admins
);

CREATE POLICY "Prevent unauthorized event updates" 
ON public.medical_events 
FOR UPDATE 
TO authenticated
USING (
  created_by = auth.uid()  -- Only creators can update their events
  OR public.is_current_user_verified_admin()  -- Or verified admins
)
WITH CHECK (
  created_by = auth.uid()  -- Only creators can update their events
...
CREATE POLICY "Prevent unauthorized event deletions" 
ON public.medical_events 
FOR DELETE 
TO authenticated
USING (
  created_by = auth.uid()  -- Only creators can delete their events
  OR public.is_current_user_verified_admin()  -- Or verified admins
);
...
BEGIN
    ...
    IF TG_OP = 'SELECT' AND (
        COALESCE(NEW.status, OLD.status) != 'approved' 
        OR COALESCE(NEW.review_notes, OLD.review_notes) IS NOT NULL 
        OR array_length(COALESCE(NEW.moderation_flags, OLD.moderation_flags), 1) > 0
    ) THEN
        INSERT INTO public.security_audit_log (
            ...
            jsonb_build_object(
                ...
                'has_review_notes', COALESCE(NEW.review_notes, OLD.review_notes) IS NOT NULL,
                'moderation_flags_count', coalesce(array_length(COALESCE(NEW.moderation_flags, OLD.moderation_flags), 1), 0),
                'access_type', TG_OP
            )
        );
    END IF;
...
BEGIN
    ...
    SELECT 
        ...
        CASE WHEN COUNT(*) >= 8 THEN 'COMPREHENSIVE' ELSE 'NEEDS_REVIEW' END::TEXT,
        CASE WHEN COUNT(*) >= 8 THEN 'LOW' ELSE 'MEDIUM' END::TEXT,
...
-- 7. Revoke any excessive permissions and ensure proper access control
