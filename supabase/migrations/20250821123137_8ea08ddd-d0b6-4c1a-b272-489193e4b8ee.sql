...
-- 7. Create audit trigger for directory access
CREATE OR REPLACE FUNCTION public.audit_directory_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DROP TRIGGER IF EXISTS directory_access_audit ON public.safe_professional_directory;
CREATE TRIGGER directory_access_audit
    AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.safe_professional_directory
    FOR EACH ROW EXECUTE FUNCTION public.audit_directory_access();
...
BEGIN
    ...
    SELECT 
        ...
        CASE WHEN EXISTS(
            ...
            WHERE tgname = 'directory_access_audit'
        ) THEN 'SECURE' ELSE 'MISSING' END::TEXT,
        'Directory access audit logging active for security monitoring'::TEXT,
        CASE WHEN EXISTS(
            SELECT 1 FROM pg_trigger 
            WHERE tgname = 'directory_access_audit'
...
-- 10. Initial population of the directory (admin function)
-- Note: This will be empty initially and needs to be populated by an admin calling refresh_safe_professional_directory()
