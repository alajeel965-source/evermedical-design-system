-- ========================================
-- ADMIN SCHEMA + RBAC MIGRATION (Complete Clean)
...
DECLARE 
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname, tablename FROM pg_policies WHERE tablename IN ('profiles', 'admin_audit') AND schemaname = 'public')
    LOOP
...
CREATE POLICY "profile_select_super_admin" 
ON public.profiles FOR SELECT 
...
CREATE POLICY "profile_update_basic_fields" 
ON public.profiles FOR UPDATE 
USING (auth.uid() = user_id AND NOT public.can_manage_users())
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "profile_update_super_admin" 
ON public.profiles FOR UPDATE 
USING (public.is_super_admin())
WITH CHECK (public.is_super_admin());

CREATE POLICY "profile_delete_super_admin" 
ON public.profiles FOR DELETE 
USING (public.is_super_admin());

-- 11. Create RLS policies for admin_audit table
CREATE POLICY "audit_select_super_admin" 
ON public.admin_audit FOR SELECT 
USING (public.is_super_admin());

CREATE POLICY "audit_select_admin_recent" 
ON public.admin_audit FOR SELECT 
USING (
...
CREATE POLICY "audit_block_updates" 
ON public.admin_audit FOR UPDATE 
USING (false);

CREATE POLICY "audit_block_deletes" 
