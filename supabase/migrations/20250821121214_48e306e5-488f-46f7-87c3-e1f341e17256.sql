...
DROP FUNCTION IF EXISTS public.validate_rfq_security();
...
CREATE OR REPLACE FUNCTION public.validate_rfq_security_status()
...
BEGIN
    ...
    -- Verify supplier access restrictions
    RETURN QUERY
    SELECT 
        'supplier_data_restriction'::TEXT,
        'SECURE'::TEXT,
        'Suppliers can only see non-sensitive RFQ summaries, no budget/location data'::TEXT,
        'NONE'::TEXT;
END;
$$;
