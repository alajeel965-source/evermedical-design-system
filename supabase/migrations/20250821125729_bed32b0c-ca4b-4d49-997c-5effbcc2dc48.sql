...
-- 9. EXPLICITLY BLOCK all UPDATE and DELETE operations - NO EXCEPTIONS
...
-- 10. Create tamper detection trigger
...
-- 11. Create function to validate audit log security
...
BEGIN
    ...
    SELECT 
        ...
        CASE WHEN EXISTS(
            ...
            AND polname = 'Block all audit log modifications'
            AND polcmd IN ('w', 'd') -- UPDATE and DELETE
            AND polqual = 'false'
        ...
        CASE WHEN EXISTS(
            ...
            AND polname = 'Block all audit log modifications'
        ) THEN 'LOW' ELSE 'CRITICAL' END::TEXT,
...
-- 12. Create function to check for audit log tampering attempts
CREATE OR REPLACE FUNCTION public.check_audit_tampering_attempts()
RETURNS TABLE(alert_type TEXT, user_id UUID, severity TEXT, details JSONB, attempt_timestamp TIMESTAMPTZ)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
    -- Only audit admins can check for tampering attempts
    IF NOT public.is_current_user_audit_admin() THEN
        RAISE EXCEPTION 'Access denied: Audit tampering monitoring restricted to audit administrators';
    END IF;
    
    -- Look for tampering attempts in the last 7 days
    RETURN QUERY
    SELECT 
        'AUDIT_LOG_TAMPERING'::TEXT,
        sal.user_id,
        'CRITICAL'::TEXT,
        sal.details,
        sal.timestamp
    FROM public.security_audit_log sal
    WHERE sal.operation LIKE 'TAMPERING_ATTEMPT_%'
      AND sal.timestamp > NOW() - INTERVAL '7 days'
    ORDER BY sal.timestamp DESC;
    
    -- Also check for suspicious patterns (multiple failed access attempts)
    RETURN QUERY
    SELECT 
        'SUSPICIOUS_AUDIT_ACCESS'::TEXT,
        sal.user_id,
        'HIGH'::TEXT,
        jsonb_build_object(
            'access_count', COUNT(*),
            'operations', array_agg(DISTINCT sal.operation),
            'time_range', MIN(sal.timestamp) || ' to ' || MAX(sal.timestamp)
        ),
        MAX(sal.timestamp)
    FROM public.security_audit_log sal
    WHERE sal.table_name = 'security_audit_log'
      AND sal.timestamp > NOW() - INTERVAL '24 hours'
    GROUP BY sal.user_id
    HAVING COUNT(*) > 10  -- More than 10 audit log accesses in 24h is suspicious
    ORDER BY COUNT(*) DESC;
END;
$$;
