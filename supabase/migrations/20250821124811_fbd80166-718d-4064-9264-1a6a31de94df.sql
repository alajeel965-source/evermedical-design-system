-- SECURITY FIX: Restrict access to sensitive organizer contact information in medical_events table

-- 1. Force RLS to ensure no bypass is possible
ALTER TABLE public.medical_events FORCE ROW LEVEL SECURITY;

-- 2. Drop existing overly permissive public access policy
DROP POLICY IF EXISTS "Public can view approved events safe data" ON public.medical_events;

-- 3. Create new restrictive policy for public access that excludes sensitive data
-- This policy will be used in conjunction with a safe public view
CREATE POLICY "Public can view approved events basic data only" 
...
-- 4. Enhance verified user policy to be more restrictive for sensitive data
DROP POLICY IF EXISTS "Verified users can view events with contact check" ON public.medical_events;

CREATE POLICY "Verified users can view approved events without sensitive contact" 
ON public.medical_events 
FOR SELECT 
TO authenticated
USING (
...
-- 5. Create function to check if user can access sensitive organizer data
...
-- 6. Create secure function to access organizer contact information
...
-- 7. Update the public_medical_events sync trigger to ensure no sensitive data leaks
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    INSERT INTO public.public_medical_events (
      ...
      NEW.subspecialities, NEW.target_audience, NEW.languages, NEW.has_cme, NEW.cme_provider,
    ...
    -- Note: Deliberately excluding organizer_email, organizer_phone, review_notes, moderation_flags
    ON CONFLICT (id) DO UPDATE SET
      ...
      subspecialties = NEW.subspecialities,
...
-- 8. Create validation function to verify security implementation
